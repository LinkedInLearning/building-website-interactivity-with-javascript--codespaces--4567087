...
$(function() {
  ...
  $('.fullheight').css('min-height', wheight);

  var resizeCork = null;
  $(window).resize(function() {
    window.clearTimeout(resizeCork);
    resizeCork = window.setTimeout(function() {
      var wheight = $(window).height(); //get height of the window
      $('.fullheight').css('min-height', wheight);
    }, 1000);
  }); //on resize
...
// Animated Scrolling
  ...
  $(window).scroll(function() {
    window.clearTimeout(resizeCork);
    scrollCork = window.setTimeout(function() {
    var windowpos = $(window).scrollTop() + topoffset;
    var $menulinks = $('nav li a');
    
    // recalculate as we scroll since these can change
    var hotelinfoTop = $('#hotelinfo').offset().top - topoffset,
        roomsTop = $('#rooms').offset().top - topoffset,
        diningTop = $('#dining').offset().top - topoffset,
        eventsTop = $('#events').offset().top - topoffset,
        attractionsTop = $('#attractions').offset().top - topoffset;

    if (windowpos > hotelinfoTop) {
      $menulinks.removeClass('active');
      $('a[href$="#hotelinfo"]').addClass('active');
    } //windowpos

    if (windowpos > roomsTop) {
      $menulinks.removeClass('active');
      $('a[href$="#rooms"]').addClass('active');
    } //windowpos

    if (windowpos > diningTop) {
      $menulinks.removeClass('active');
      $('a[href$="#dining"]').addClass('active');
    } //windowpos

    if (windowpos > eventsTop) {
      $menulinks.removeClass('active');
      $('a[href$="#events"]').addClass('active');
    } //windowpos

    if (windowpos > attractionsTop) {
      $menulinks.removeClass('active');
