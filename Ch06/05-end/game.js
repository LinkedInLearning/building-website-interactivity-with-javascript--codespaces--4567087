...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const intersectsASlot = (tileEl) => {
        const roundableProps = ['left', 'top', 'bottom', 'right', 'width', 'height'];
        ...
        for (const slotEl of slotsRegion.querySelectorAll('.slot')) {
            ...
            if (slot.left < tile.left + tile.width &&
    ...
    const mouseUpHandler = () => {
        ...
        if (targetSlot) {
            ...
            const tileTop = parseInt(slotCoords.top, 10) - parseInt(boardRect.top, 10);
            const tileLeft = parseInt(slotCoords.left, 10) - parseInt(boardRect.left, 10);
